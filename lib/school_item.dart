...
class SchoolItemPage extends StatefulWidget {
  final Function(Map<String, dynamic>) onAddToCart;

  const SchoolItemPage({Key? key, required this.onAddToCart}) : super(key: key);

  @override
  State<SchoolItemPage> createState() => _SchoolItemPageState();
}

class _SchoolItemPageState extends State<SchoolItemPage> {
  ...
  void _startLoadingData() async {
    ...
    _streamSubscription = FirebaseFirestore.instance
        ...
        .where('category', isEqualTo: 'School Items')
        .snapshots()
        .listen((snapshot) {
      if (!mounted) return;

      print('📦 Loaded ${snapshot.docs.length} school items');
              ...
              child: Padding(
                ...
                child: Row(
                  children: [
                    ...
                    Text(
                      AppLocalization.getText('schoolItems'),
                      style:
          ...
          Padding(
            ...
            child: TextField(
              ...
              decoration: InputDecoration(
                hintText: AppLocalization.getText('searchSchoolItems'),
  ...
  Widget _buildContent() {
      ...
      return Center(
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 16),
            Text(
              AppLocalization.getText('noSchoolItemsFound'),
              style: const TextStyle(
                fontSize: 16,
    ...
    return ListView.builder(
      ...
      itemBuilder: (context, index) {
        ...
        final fullProductData = {'id': doc.id, ...product};
        return _SchoolItemListItem(
          key: ValueKey(doc.id),
...
class _SchoolItemListItem extends StatefulWidget {
  final Map<String, dynamic> product;
  final Function(Map<String, dynamic>) onAddToCart;
  final VoidCallback onImageTap;

  const _SchoolItemListItem({
    Key? key,
  ...
  @override
  _SchoolItemListItemState createState() => _SchoolItemListItemState();
}

class _SchoolItemListItemState extends State<_SchoolItemListItem> {
  int _quantity = 1;
  late String _unit;
  ...
  void initState() {
    super.initState();
    _unit = widget.product['uom']?.toLowerCase() ?? 'piece';
  ...
  Widget _buildProductImage(String? imageUrl) {
    Widget fallbackImage = Container(
      ...
      child: Center(
        child: Icon(Icons.school, color: Color(0xFF5B35B8), size: 24),
      ),
    );
  ...
  @override
  Widget build(BuildContext context) {
    final name = widget.product['name'] ?? 'School Item';
          ...
          Row(
            ...
            children: [
              _buildQuantityButton(
                  icon: Icons.remove, onTap: () => _adjustQuantity(-1)),
              Container(
                width: 35,
                height: 24,
              ...
              _buildQuantityButton(
                  icon: Icons.add, onTap: () => _adjustQuantity(1)),
              const SizedBox(width: 12),
              ElevatedButton(
